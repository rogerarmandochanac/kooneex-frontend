...
class AuthService {
    ...
    try {
      final response = await http
          ...
          .timeout(const Duration(
              seconds: 10)); // Evita que se quede cargando infinitamente
  ...
  Future<Map<String, dynamic>> register({
    ...
    try {
      var request = http.MultipartRequest(
          'POST', Uri.parse('$_baseUrl/usuarios/registro/'));
      ...
      return {
        'success': false,
        'message':
            'No se pudo establecer conexión. Revisa tu internet o los datos móviles.'
      };
    } on TimeoutException {
      return {
        'success': false,
        'message':
            'La carga de la foto está tardando demasiado. Intenta con una conexión más estable.'
      };
    } on HttpException {
      return {
        'success': false,
  ...
  Future<Map<String, dynamic>> verificarEstadoViaje() async {
    try {
      ...
      if (token == null) return {'mensaje': 'none'};
  ...
  Future<List<dynamic>> obtenerOfertas() async {
    try {
      ...
      final token = prefs.getString('access_token');
      final viajeId = prefs
          .getInt('current_viaje_id'); // El ID que guardamos al crear el viaje
  ...
  Future<bool> actualizarUbicacion(double lat, double lng) async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
            '$_baseUrl/usuarios/actualizar_ubicacion/'), // Ajusta a tu endpoint de perfil
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  Future<bool> cancelarOfertaPropia() async {
    final prefs = await SharedPreferences.getInstance();
    final token =
        prefs.getString('access_token'); // Usamos el ID del viaje actual
    final viajeId = prefs.getInt('current_viaje_id');

    final response = await http.delete(
      Uri.parse(
          '$_baseUrl/ofertas/$viajeId/rechazar/'), // Tu endpoint de Django
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
...
// lib/services/auth_service.dart
  ...
  Future<bool> cambiarEstadoViaje(int viaje) async {
    ...
    final response = await http.patch(
      Uri.parse(
          '$_baseUrl/viajes/$viaje/en_curso/'), // Endpoint: .../viajes/ID/en_curso/
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
