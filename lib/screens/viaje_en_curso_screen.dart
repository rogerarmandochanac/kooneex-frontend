...
class _ViajeEnCursoScreenState extends State<ViajeEnCursoScreen> {
  ...
  void _optimizarTrazoRuta(latLng.LatLng posicionActual) {
    ...
    for (int i = 0; i < _puntosRuta.length; i++) {
      double d = distance.as(latLng.LengthUnit.Meter, posicionActual, _puntosRuta[i]);
      if (d < umbralMetros) {
        puntosAEliminar = i + 1;
      } else {
        break; 
...
Future<void> _obtenerRutaOSRM(double cLat, double cLon) async {
  ...
  Widget build(BuildContext context) {
    ...
    double cLat = double.tryParse(_viaje!['conductor_lat']?.toString() ?? "0") ?? 0;
    double cLon = double.tryParse(_viaje!['conductor_lon']?.toString() ?? "0") ?? 0;
    double pLat = double.tryParse(_viaje!['pasajero_lat']?.toString() ?? "0") ?? 0;
    double pLon = double.tryParse(_viaje!['pasajero_lon']?.toString() ?? "0") ?? 0;
          ...
          FlutterMap(
            ...
            children: [
              ...
              if (_puntosRuta.isNotEmpty)
                PolylineLayer(
                  polylines: [
                    Polyline(points: _puntosRuta, strokeWidth: 5, color: const Color(0xFFF7931E)),
                  ],
                ),
              MarkerLayer(
                markers: [
                  Marker(
                    point: _posicionActualConductor ?? latLng.LatLng(cLat, cLon),
                    width: 60, height: 60,
                    child: Transform.rotate(
                      angle: _bearing * (3.14159 / 180),
                      child: const Icon(Icons.motorcycle, color: Color(0xFFF7931E), size: 40),
                    ),
                  ),
                  Marker(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (_viaje?['referencia'] != null && _viaje!['referencia'].toString().isNotEmpty) _buildReferenciaSection(),
            ],
          ),
        ),
      ),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: _marcarCompletado, child: const Text("FINALIZAR")),
        ],
      ),
    );
  }
